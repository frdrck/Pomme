...
var Webcam =
	...
	receive: function (url)
		{
		filename = url.replace(SITE_URL + "img/webcam/", "")
...
/* Temporary mobile build banner — remove PommeMobileBuildModal and the joinCallback call below when done. */
var PommeMobileBuildModal =
	...
	show: function ()
		...
		p.appendChild(document.createTextNode("This screen means you loaded client bundle "))
...
var Game =
	...
	redirect_to: function (redirect)
		{ document.location = SITE_URL + redirect },
	...
	setupStates: function ()
		...
		Game.states[STATE_IDLE] = function (data)
			...
			Game.hideCards ()
			if (data['round'] > 0)
				return ["Nobody here -- Send the link to your friends!<br/><input type='text' id='copy_url' readonly value='" + SITE_URL + Game.name+"'/>", " "]
			else
				return ["Nobody here -- Send the link to your friends!<br/><input type='text' id='copy_url' readonly value='" + SITE_URL + Game.name+"'/>", " "]
			}
