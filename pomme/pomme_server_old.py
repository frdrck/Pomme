...
class Game:
	...
	def api_join (self, args):
		...
		resp['cards'] = self.players[args['username']].cards
...
#		DB.game_edit (args, game.id)
	...
	def find_an_active_game (self):
		for name,game in self.games.iteritems():
			if name != "bigapple" and not game.private and len(game.active) > 0 and len(game.active) < 8:
				return game.path
		newgame = self.api_game_newgame()
		return newgame['path']
	def api_game_active (self, args):
		path = self.find_an_active_game()
		return { 'path': path }
